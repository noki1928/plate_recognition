...
class LicensePlateRecognition:
    ...
    def __init__(
        ...
        path_to_rot: str = r"models\rotation\rotation.pth",
    ) -> None:
        """
        ...
        try:
            self.det = Detection(path_to_det, path_to_rot)
    ...
    async def get_plates(
        ...
        """Теперь принимает и путь, и изображение в памяти."""
        if self.det is None:
            logger.error("Модель детекции не инициализирована")
            return ""

        try:
            img_rois = await self.det.get_rois(source)
            if not img_rois:
    ...
    async def get_plates_with_validation(
        self,
        source: Union[str, Path, np.ndarray, PILImage.Image],   # ← тоже изменили
    ) -> dict:
        plates = await self.get_plates(source)
